...
(function() {
  $(document).ajaxComplete(function() {
    ...
    return $('#season_id').change(function() {
      var options, season;
      season = $('#season_id :selected').text();
      options = $(episodes).filter("optgroup[label='" + season + "']").html();
      if (options) {
        return $('#clip_episode_id').html(options);
      } else {
        return $('#clip_episode_id').empty();
      }
    });
  });

  $(document).ready(function() {
    return $('.youtube-thumbnail').on('click', function() {
      return $(this).parent().html('<iframe width="560" height="315" src="//www.youtube.com/embed/' + this.id + '?autoplay=1&showinfo=0" frameborder="0" allowfullscreen></iframe>');
    });
  });
...
(function() {
  $(document).ajaxComplete(function() {
    ...
    return $('#season_id').change(function() {
      ...
      options = $(episodes).filter("optgroup[label='" + season + "']").html();
      if (options) {
        return $('#quote_episode_id').html(options);
      } else {
        return $('#quote_episode_id').empty();
      }
    });
  });
